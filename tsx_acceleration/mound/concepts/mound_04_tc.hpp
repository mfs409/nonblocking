/**
 *  Mound Refinement 4: Try/Catch
 *
 *  [mfs] This is a syntactic refinement.  The use of try/catch decreases the
 *        lines of code.  Every atomic block is an implicit try.
 */
...
bool push_cavity_down(Node* N, int nts, Node* C, int cts)
{
    // validate N
    if (N.ts != nts)
        return false;
...
void fill_cavity_tc(Node* N)
{
    ...
    while (true) {
        ...
        atomic {
            ...
            fill_cavity_tc(N->right); continue;
        }
        catch (LeftCavity) {
            fill_cavity_tc(N->left); continue;
        }
        ...
        if ((rv <= lv) && (rv < nv)) {
            atomic {
                if (push_cavity_down(N, nts, N->right, rts)) return;
            }
        }
        // pull from left?
        else if ((lv <= rv) && (lv < nv)) {
            atomic {
                if (push_cavity_down(N, nts, N->left, lts)) return;
            }
        }
