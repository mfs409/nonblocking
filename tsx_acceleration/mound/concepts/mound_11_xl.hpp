/**
 *  Mound Refinement 11: eXtra Lazy: we are going to be really aggressive
 *  here about *not* filling cavities.
 *
 *  [mfs] The only changes should be in the insert function, but they require
 *  a lot of thought and haven't been made yet.
 */
...
void fill_cavity_xl(Node* N)
{
  ...
  ensure_not_leaf:
    if (is_leaf(N)) {
        ...
        if (NN.is_cavity) {
            if (!ATOMIC_C2S1I(N, NN, (NN.list, false, NN.ts + 1),
    ...
    while (true) {
        ...
        // NB: siblings can never both be cavities, so we don't need try to
        // prevent control flow by cleaning L and R in the same iteration
