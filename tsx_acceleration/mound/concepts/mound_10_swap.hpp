/**
 *  Mound Refinement 10: use a list swap when cleaning cavities
 */
...
void fill_cavity_swap(Node* N)
{
    ...
    while (true) {
        // [lyj] order the READ before checking whether N is leaf, so we don't
        // maintain a global LEAFINFO field. if the CAS succeeds, we are safe
        // to return even if some one expand the mound such that N is no
        // longer a leaf, in which case, a concurrent insert (which thinks i'm
        // not a leaf) either conflict with my CAS, or absolutely disjoint
        // with my working set.
        N->READ(&NN);
        // return if N is already not a cavity
        if (!NN.cavity)
            return;
        // cannot take the short path if N is not leaf
        if (!is_leaf(N))
            break;
        // remove the cavity on leaf using single CAS
    ...
    while (true) {
        ...
        if ((rv <= lv) && (rv < nv)) {
            // swap R and N lists
            if (ATOMIC_C2S2(N, NN, (RR.list, false, NN.ts + 1),
                            R, RR, (NN.list, true,  RR.ts + 1)))
                return;
            continue;
        }
        // pull from left?
        if ((lv <= rv) && (lv < nv)) {
            // swap L and N lists
            if (ATOMIC_C2S2(N, NN, (LL.list, false, NN.ts + 1),
                            L, LL, (NN.list, true,  LL.ts + 1)))
                return;
            continue;
        }
 ...
 */
int extract_min_swap()
{
    List* extracted = NULL;
    while (true) {
        nodecache_t RR = ROOT->READ();
        if (RR.iscavity) {
            fill_cavity_swap(ROOT);
            continue;
        }
        if (RR.list == NULL)
