    ...
    bool insert(int key)
    {
      ...
      retry:
        ...
        if (status == _XBEGIN_STARTED) {
            ...
            w.all = t->buckets[i];
            b = w.fields.ptr;
            // if the b is empty, goto slow path
            if (b == NULL || IS_MARKED(b)) _xabort(42);
            for (int i = 1; i <= b[0]; i++) {
                if (b[i] == key) {
                    result = -(b[0] + 1);
                    goto commit;
            ...
            if (b[0] >= MIN_ALLOC_LEN) _xabort(42);
            b[0]++;
            b[b[0]] = key;
            w.fields.ctr++;
            t->buckets[i] = w.all;
